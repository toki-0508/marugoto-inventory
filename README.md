...
- `item_type` には `物品` / `電子機器` / `消耗品` を入れる
- 互換のため `equipment` / `electronics` / `consumable` でも動作する
- `電子機器` は取扱注意品として分類するための種別で、貸出・返却の扱いは `物品` と同じ
